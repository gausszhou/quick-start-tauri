...
var RegExpTools = (function () {
  ...
  var _createTag = function (type, item) {
    ...
    for (var i = 0, len = item.text.length; i < len; i++) {
      tags.push("<" + type + ' data-id="' + ID_PREFIX + item.index + '">' + item.text.charAt(i) + "</" + type + ">");
    }
    return tags.join("");
  };
  ...
  var _init = function () {
    ...
    $("#regText,#srcCode").keyup(_dealRegMatch).change(_dealRegMatch).bind("paste", _dealRegMatch);
